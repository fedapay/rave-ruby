...
module RaveRuby
  class ChargeBase < Base
    ...
    def update_payload(suggested_auth, payload, **keyword_args)
      ...
      raise SuggestedAuthError, 'Please pass the suggested auth.' unless keyword_args.key?(auth_type)

      # if the authorization type is equal to address symbol, update with the required parameters
      if auth_type == :address
        required_parameters = %w[billingzip billingcity billingaddress billingstate billingcountry]
      ...
      payload.merge!({'suggested_auth' => suggested_auth, auth_type.to_s => keyword_args[auth_type]})
    end
    ...
    def handle_charge_response(response)
      ...
      authurl = if charge_response['data']['authurl'] == 'N/A'
                  'N/A'
                else
                  charge_response['data']['authurl']
                end
